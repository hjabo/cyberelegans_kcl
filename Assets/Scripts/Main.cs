...
public class Main : MonoBehaviour {
    ...
    void Update()
    {
        if (Time.time > lastSecond + 0.1f) {
            ...
            UIMan.Inst.SetText(4, "Active Neurons (F):", Globals.neuron_active);
        ...
        UIMan.Inst.SetText(0, "Step:" , TotalStep);
        if (TotalStep%100==0&& TotalStep!=0)
        {
            ScreenShot.Inst.Capture();
        }
    }

    void UpdateLogic()
    {
        if(Globals.applyGravity)TotalStep++;
