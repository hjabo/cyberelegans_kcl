...
class Elegans
{
    ...
    public Elegans(float length, int size)
    {
        ...
        for (int j = 0; j < mNeuron.Count; j++)
        {
            //mNeuron[j].income = Mathf.Max(UnityEngine.Random.Range(-20.0f, 0.05f),0);
            //mNeuron[j].income = 0.1f;
            mNeuron[j].income = UnityEngine.Random.Range(0.0f, 0.01f);
        }
        for (int j = 0; j < mMuscle.Count; j++) mMuscle[j].income = UnityEngine.Random.Range(0.0f, 0.1f);
    ...
    public void UpdateLogic(float dt) {
        if (mPoint[0].checkCol()%2==1)
        {
            kclpoint += 8;
            Debug.Log("KCl과의 충돌 감지");
        ...
        if (kclpoint > 0)
        {
            for (int i = 0; i < mNeuron.Count; i++)
            {
                if (mNeuron[i].name == "ASER")
                {
                    mNeuron[i].income += 2.0f;
                    kclpoint--;
                    break;
                }
            }
        }
        if (mPoint[0].checkCol() >=2)
        {
            ashpoint += 2;
            if (ashpoint >= 100)
            {
        ...
        if (ashpoint > 0)
        {
            for (int i = 0; i < mNeuron.Count; i++)
            {
                if (mNeuron[i].name == "ASHL" || mNeuron[i].name == "ASHR" || mNeuron[i].name == "FLPL" || mNeuron[i].name == "FLPR" || mNeuron[i].name == "OLQDL" || mNeuron[i].name == "OLQDR" || mNeuron[i].name == "OLQVL" || mNeuron[i].name == "OLQVR") //ASH, FLP, OLQ
                {
                    mNeuron[i].income += 0.8f;
                }
            }
        }
        //if(f9) {
        ...
        if (Globals.muscleSimulate) {
            ...
            for(int i = 0; i < mMuscle.Count; ++i) {
                ...
                if((m_name.IndexOf("VL") != -1) || (m_name.IndexOf("VR") != -1)) {
                    neuro_signal = (0.3f * (Mathf.Sin(3 * Mathf.PI * j / 24 - Mathf.PI / 2 - 0.5f * 1.3f*time * Globals.direction) + 1.0f) / 2);
            ...
            for(int i = 0; i < mNeuron.Count; i++) {
                ...
                if((n_name[0] == 'V' && n_name[1] == 'B') || (n_name[0] == 'D' && n_name[1] == 'B')) {
                    ...
                    if(n_name[0] == 'V' && n_name[1] == 'B') {
                        ...
                        mNeuron[i].ReceiveSignal(100*neuro_signal);
                    }else if(n_name[0] == 'D' && n_name[1] == 'B') {
